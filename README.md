...
# 작업 후 느낀 점 ✅
    ...
    - 보이지 않는 요구사항의 존재에 대해 알 수 있는 기회였던 것 같다. 예를 들어, 배달 어플에 존재하는 쿠폰 시스템같은 경우 그 쿠폰들이 전부 유효기간이 존재하는데, 유효기간 이전에 시행한 주문을 쿠폰의 유효기간이 끝나고 취소한다면 어떻게 해야하는가와 같은 당장 주어진 표면적인 요구사항뿐 아닌, 이로 인해 발생하는 여파들을 생각하는 다음 단계를 생각하는 것이 개발 과정에서 중요하게 작용한다는 것을 배웠다.
      
    - 죽은 프로젝트지만 개발 난이도가 당시 기준으로 낮게 느껴지는 편은 아니었기에 재밌게 했다. 특히 기능이 추가되면서 기존 엔티티의 구조를 건드리기 시작했는데, 이게 생각보다 손이 많이 가고 복잡하다. 왜 확장성 있는 설계를 고민해야 하는가에 대한 교훈을 직접 경험하게 해주었다.
